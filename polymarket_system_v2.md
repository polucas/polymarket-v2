...
>
> **v2.10 changes:**
> - **`MARKET_FETCH_PAGES` default = 10, `MARKET_PAGE_SIZE` = 100 (post-2026-05-20 pagination fix):** Polymarket Gamma API silently caps `limit` at 100 per request — anything larger is silently truncated. Original `MARKET_PAGE_SIZE=500` triggered the code-level early-break (`if len(page_markets) < page_size: break`), terminating the loop after page 1 and fetching only 100 markets/scan. Fix: set `MARKET_PAGE_SIZE=100` (match API cap) + `MARKET_FETCH_PAGES=10` via offset pagination → 1,000 markets/scan as intended.
